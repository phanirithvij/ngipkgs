...
# if the update script is run via using flakes, `nix develop`
# then the SCRIPT_PATH will start with /nix/store (in this case)
...
src="$(nix-build --no-link "$ROOT_DIR" -A repath-studio.src)"
...
set +e
new_mvn_hash="$(
  nix-build --no-link -A repath-studio.passthru.mavenRepo "$ROOT_DIR" 2>&1 >/dev/null |
